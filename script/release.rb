...
VERSION_REGEX = /^(0|[1-9][0-9]*)$/.freeze
PRE_VERSION_REGEX = /^(0|[1-9][0-9]*)(?:[.]pre)?$/.freeze
...
gem_file = "git-coauthor-#{version}.gem"
...
File.write(
  ...
  File.read('lib/git_coauthor/version.rb').sub(/VERSION = .*/, "VERSION = '#{version}'")
)
...
uri = URI('https://api.github.com/repos/nicholasdower/git-coauthor/releases')
body = {
  tag_name: "v#{version}",
...
uri = URI("https://uploads.github.com/repos/nicholasdower/git-coauthor/releases/#{release_id}/assets?name=#{gem_file}")
headers = {
  'Accept' => 'application/vnd.github+json',
  'Authorization' => "Bearer #{ENV.fetch('GITHUB_TOKEN')}",
...
data = "scheme=git&url=https://github.com/nicholasdower/git-coauthor&commit=v#{version}"
